...
const DEFAULT_THEME: String = "catppuccin mocha"
...
func _ready() -> void:
	...
	Settings.settings_window.secret_signaled.connect(func(): EditorThemeManager.change_theme(DEFAULT_THEME, true))
...
func change_theme_from_text(use_cache: bool = true, theme_text: String = "", random: bool = false) -> void:
	...
	Icons.icon_color = colors.text_color
	...
	t.set_stylebox(&"button_hover", &"Tree", StyleBoxUtil.new_flat(_color_adjust(colors.background_color, -contrast * 0.05), [4], [4]))
	t.set_stylebox(&"button_pressed", &"Tree", StyleBoxUtil.new_flat(_color_adjust(colors.background_color, -contrast * 0.1), [4], [4]))
	t.set_stylebox(&"custom_button_hover", &"Tree", t.get_stylebox(&"button_hover", &"Tree"))
	t.set_stylebox(&"custom_button_pressed", &"Tree", t.get_stylebox(&"button_pressed", &"Tree"))

	var guide_color := Color(colors.text_color, 0.5)

	t.set_color(&"font_color", &"Tree", colors.text_color)
	t.set_color(&"font_hovered_color", &"Tree", colors.text_color)
